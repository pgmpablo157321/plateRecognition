...
img_ = cv2.imread('../imagenes/img_17.jpg',1)
...
cv2.setTrackbarPos('umbral_azul','Ventana Trackbar', 30)
cv2.setTrackbarPos('umbral_rojo','Ventana Trackbar',60)
cv2.setTrackbarPos('umbral_verde','Ventana Trackbar',90)
...
while (True):
    ...
    U2=cv2.getTrackbarPos('umbral_rojo','Ventana Trackbar')
    U3=cv2.getTrackbarPos('umbral_verde','Ventana Trackbar')

    ret, img_bin_b = cv2.threshold(add[:,:,0],U1,255,cv2.THRESH_BINARY_INV)
    ret2, img_bin_g = cv2.threshold(add[:,:,1],U2,255,cv2.THRESH_BINARY_INV)
    ret2, img_bin_r = cv2.threshold(add[:,:,2],U3,255,cv2.THRESH_BINARY_INV)

    result = cv2.bitwise_and(cv2.bitwise_and(img_bin_g,img_bin_r), img_bin_b)
